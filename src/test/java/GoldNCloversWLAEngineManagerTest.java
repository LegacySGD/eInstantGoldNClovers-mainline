...
public class GoldNCloversWLAEngineManagerTest
{
	...
	{
		logger = Logger.getLogger(GoldNCloversWLAEngineManagerTest.class);
		gameParams = XMLUtil.parseResource("paymodel/GoldNCloversIW/GameParams.xml").getDocumentElement();
		previousOutcome = XMLUtil.parseResource("paymodel/GoldNCloversIW/InitOutcome.xml").getDocumentElement();
			...
			throws Exception
	{
		...
		TestODEProxy.setJsonResponse("{'prizeValue':0}");
		...
		assertEquals("{'revealData':'test message'}", JXPathUtil.getString(outcome, "//JSONOutcome[@name='ODERevealData']/text()"));
		...
		throws Exception
	{
		...
		// Scenario
		playerInput = new PlayerInputBuilder().appendActionInput("play").build();
		TestODEProxy.setJsonResponse("{'prizeValue':100}");
		outcome = OutcomeUtil.getSignedData(outcome, "Outcome");
		outcome = manager.play(InputUtil.buildInput(gameParams, XMLUtil.getElement(outcome), playerInput));
		logger.debug(outcome);
		...
		assertEquals("Reveal", JXPathUtil.getString(outcome, "//NextStage/text()"));
		assertEquals("{'revealData':'msg1'}", JXPathUtil.getString(outcome, "//JSONOutcome[@name='ODERevealData']/text()"));
		assertSettledPendingPayout(outcome, 0, 2, 0);
		...
		assertEquals("{'revealData':'msg2'}", JXPathUtil.getString(outcome, "//JSONOutcome[@name='ODERevealData']/text()"));
		assertEquals("Wager", JXPathUtil.getString(outcome, "//NextStage/text()"));
